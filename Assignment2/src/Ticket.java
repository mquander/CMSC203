...
public class Ticket {
	...
	public double calculateFine() {
		double fine = 0;
		if(getSpeed() > getSpeedLimit() && getSpeed() <= getSpeedLimit() + 10) {
			if(isWorkZone() || (isWorkZone() && isSchoolZone())) {
				fine = 250.0;
			}
		...
		if(getSpeed() > getSpeedLimit() +10 && getSpeed() <= getSpeedLimit() + 20) {
			if(isWorkZone() || (isWorkZone() && isSchoolZone())) {
				fine = 350.0;
			}
			else if(isSchoolZone()) {
		...
		else if(getSpeed() > getSpeedLimit() +20 && getSpeed() <= getSpeedLimit() + 30) {
			
			fine = 450.0;
		}
		else if(getSpeed() > getSpeedLimit() +30 && getSpeed() <= getSpeedLimit() + 40) {
			
			fine = 495.0;
		}
		else if(getSpeed() > getSpeedLimit() + 40) {
